...
void HSearchContext::setSearchTerm(QString s) {
    ...
    while(ui->widget_albums->layout()->count()) {
        delete ui->widget_albums->layout()->takeAt(0)->widget();
    }
    while(ui->widget_artists->layout()->count()) {
        delete ui->widget_artists->layout()->takeAt(0)->widget();
    }
    while(ui->widget_tags->layout()->count()) {
        delete ui->widget_tags->layout()->takeAt(0)->widget();
    }
    while(ui->widget_tracks->layout()->count()) {
        delete ui->widget_tracks->layout()->takeAt(0)->widget();
    }
    while(ui->widget_usersAndGroups->layout()->count()) {
        delete ui->widget_usersAndGroups->layout()->takeAt(0)->widget();
    }
    while(ui->widget_venues->layout()->count()) {
        delete ui->widget_venues->layout()->takeAt(0)->widget();
    }
...
void HSearchContext::getMoreAlbums(QString s) {
    ...
    connect(this,SIGNAL(searchTermChanged()),&loop,SLOT(quit()));
    loop.exec();
    if(s_term!=s) {
        delete reply;
        return;
